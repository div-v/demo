const apikey = "6a05b6c33d9c465047a9038663fd9ecd";
...
const apiPaths = {
    ...
    searchOnYoutube: (query) => `https://www.googleapis.com/youtube/v3/search?part=snippet&q=${query}&key=AIzaSyCoBLbA4e7ANiWkndWaom0LclPyz4ucTQg`
}
...
function fetchAndbuildMovieSection(fetchUrl, categoryName){
    ...
    .then(res => {
        ...
        if (Array.isArray(movies) && movies.length) {
            buildMoviesSection(movies.slice(0,40), categoryName);
        }
        return movies;
    })
    .catch(err=>console.error(err))
...
window.addEventListener('load',function() {
    ...
    window.addEventListener('scroll', function(){
        ...
        if (window.scrollY > 30) header.classList.add('black-bg')
